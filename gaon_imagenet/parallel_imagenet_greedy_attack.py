...
class LazyGreedyAttack:
    def __init__(self, sesses, models, epsilon, loss_func):
    ...
    def ldg_block_seg(self, block_results, x_adv, y, sess, model, ith_thread, block, x_m, x_p, yk=0, rho=0, Resize = params.resize):
        ...
        model_x = tf.placeholder(tf.float32, (None, 299, 299, 3))
        model_y = tf.placeholder(tf.int64, None)

        logits, predictions = model(sess, model_x, params.model_dir)
        y_xent = tf.nn.sparse_softmax_cross_entropy_with_logits(
            logits = logits, labels = model_y)
        correct_prediction = tf.equal(predictions, model_y)
        ...
        cur_m = sess.run(loss, feed_dict={model_x:block_x_m,
                                          model_y:y})

        cur_p = sess.run(loss, feed_dict={model_x:block_x_p,
                                          model_y:y})
        ...
        while not queue.empty():
            ...
            losses, correct_predictions = sess.run([loss, correct_prediction],
                                                  feed_dict={model_x: img_batch,
                                                             model_y: y_batch})

            num_queries += 2
            if params.early_stop == 'y':
                success = np.array([0, 1])[np.invert(correct_predictions)]
                for i in success:
    ...
    def perturb(self, x_nat, y, sesses, ibatch):
        ...
        while(iter_round < params.admm_iter):
                ...
                if num_running == params.gpus:
                    ...
                    if self.success:
                        ...
                        correct_predictions = sess.run(correct_prediction,
                                                              feed_dict={model_x: img_batch,
                                                                         model_y: y_batch})
                        success = np.array([idx for idx in range(len(correct_predictions))])[np.invert(correct_predictions)]
                        for k in success:
            ...
            if params.early_stop == 'y':
                ...
                num_corrects = sess.run(num_correct,
                                              feed_dict={model_x: x_adv,
                                                         model_y: y})
...
def run_attack(x_adv, model, sess, x_full_batch, y_full_batch, percentage_mean):
    ...
    for ibatch in range(num_batches):
        ...
        dict_adv = {model_x: x_batch,
                    model_y: y_batch}
        cur_corr, correct_predictions = \
            sess.run([model.num_correct, model.correct_prediction],
                     feed_dict=dict_adv)
        total_corr += cur_corr
