...
if __name__ == '__main__':
  ...
  while count < args.sample_size:
    ...
    if success:
      ...
      tf.logging.info('Attack success, avg queries: {:.4f}, med queries: {}, avg per-gpu queries: {:.2f}, success rate: {:.4f}, avg time: {:.4f}'.format(
      ...
      average_parallel_queries = 0 if len(total_parallel_queries) == 0 else np.mean(total_parallel_queries)
      average_time = 0 if len(total_times) == 0 else np.mean(total_times)
      tf.logging.info('Attack fail, avg queries: {:.4f}, med queries: {}, avg per-gpu queries: {:.2f}, success rate: {:.4f}, avg time: {:.4f}'.format(
        average_queries, median_queries, average_parallel_queries, total_num_corrects / count, average_time))
