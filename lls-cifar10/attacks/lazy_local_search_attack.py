...
class LazyLocalSearchAttack(object):
  """Lazy Local Search Attack with Hierachical Method and Mini-batch Technique"""
  ...
  def _perturb_image(self, image, noise):
    """Perturb an image with a noise, and clip the value of the perturbed image into [0, 255] 
    
    Args:
      image: numpy array with size [1, 32, 32, 3], original image
      noise: numpy array with size [1, 32, 32, 3], noise

    Returns:
      adv_image: numpy array with size [1, 299, 299, 3], perturbed image
    """
  ...
  def _split_block(self, upper_left, lower_right, block_size):
    """Split an image into blocks with block_size. Note that a block consists of 
  ...
  def perturb(self, image, label, index, sess):		
    ...
    Returns:
      adv_image: numpy array with size [1, 32, 32, 3], adversarial image
