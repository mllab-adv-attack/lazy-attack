...
_INCEPTION_ADV_CHECKPOINT_NAME = 'ens4_adv_inception_v3.ckpt'
...
INCEPTION_ADV_CHECKPOINT_PATH = os.path.join(
    os.path.dirname(__file__),
    'data',
    _INCEPTION_ADV_CHECKPOINT_NAME
)
...
def model(sess, image, path=None):
    ...
    if not _inception_initialized:
        if path == 'nat':
            print("restoring nat model")
            optimistic_restore(sess, INCEPTION_CHECKPOINT_PATH)
        elif path == 'adv':
            print("restoring adv model")
            optimistic_restore(sess, INCEPTION_ADV_CHECKPOINT_PATH)
        else:
            raise Exception
