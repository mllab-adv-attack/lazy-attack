...
with tf.Session() as sess:
    ...
    for ii in range(max_num_training_steps):
        ...
        start = timer()
        _, loss, safe_pgd_acc_batch, orig_acc_batch, safe_acc_batch, \
        x_safe, x_safe_pgd = \
            sess.run([train_step, total_loss, safe_pgd_acc, orig_acc, safe_acc,
                      full_model.x_safe, full_model.x_safe_pgd], feed_dict=nat_dict)
        end = timer()
        assert 0 <= np.amin(x_safe) and np.amax(x_safe) <= 255.0
        assert 0 <= np.amin(x_safe_pgd) and np.amax(x_safe_pgd) <= 255.0

        l2_dist = np.mean(np.linalg.norm((x_batch-x_safe).reshape(training_batch_size, -1)/255, axis=1))
